...
class NestedAudioNode {
    ...
    if (isArray(templateApiArray)) {
      ...
      templateApiArray.forEach( (apiItem, idx) => {
        ...
        if (isArray(apiItem)) {
          ...
          if (isString(apiLabel) && isString(innerLabel) && arrayIndexPopulated(this.contents, innerIdx)) {
            ...
            if (innerNode.isNested) {
              ...
              if (isAudioParam(innerItem)) {
                ...
                innerConst = this.makeConstObj(innerNode, innerLabel, innerItem)
              } else {
  ...
  addToContents({library, addItem, idx, loc}) {
    ...
    let innerNode = this.contents[idx] = null
    let innerType = null
    let innerInit = []
    if (isString(addItem)) {
    ...
    if (innerType === 'Tone.Master') {
      ...
      return
    } else if (innerType === 'Tone.Transport') {
